...
namespace CesiumForUnity
{
    ...
    public class CesiumCameraController : MonoBehaviour
    {
        ...
        //@reviser lijuhong 添加rotationSpeed属性
        [SerializeField]
        [Min(0.0f)]
        ...
        public float rotationSpeed
        {
            ...
            set => this._rotationSpeed = Mathf.Max(value, 0.0f);
        }

        //@reviser lijuhong 添加panMoveSpeed属性
        [SerializeField]
        [Min(0.0f)]
        private float _panMoveSpeed = 2.0f;
        /// <summary>
        /// The pan move speed of this controller when movement is enabled.
        /// If movement is disabled, this value will not be used.
        /// </summary>
        public float panMoveSpeed
        {
            get => this._panMoveSpeed;
            set => this._panMoveSpeed = Mathf.Max(value, 0.0f);
        }

        //@reviser lijuhong 添加mouseScrollSpeed属性
        [SerializeField]
        [Min(0.0f)]
        private float _mouseScrollSpeed = 10.0f;
        /// <summary>
        ...
        {
            get => this.mouseScrollSpeed;
            set => this._mouseScrollSpeed = Mathf.Max(value, 0.0f);
        }
        ...
        //@reviser lijuhong 与鼠标滚轮事件冲突，注释掉speedChangeAction
        ...
        private void HandlePlayerInputs()
        {
            ...
            //@reviser lijuhong 添加inputPanForward、inputPanRight属性，用于鼠标左键移动相机
            float inputPanForward = 0;
            float inputPanRight = 0;
            //@reviser lijuhong 判断鼠标左键是否按下
            if (Mouse.current.leftButton.isPressed)
            {
                if (inputRotateVertical > 1)
                    inputPanForward = -this._panMoveSpeed;
                else if (inputRotateVertical < -1)
                    inputPanForward = this._panMoveSpeed;
                if (inputRotateHorizontal > 1)
                    inputPanRight = -this._panMoveSpeed;
                else if (inputRotateHorizontal < -1)
                    inputPanRight = this._panMoveSpeed;
            }

            //@reviser lijuhong 判断鼠标右键是否按下
            bool enableRotation = this._enableRotation && Mouse.current.rightButton.isPressed;

            //@reviser lijuhong 获取鼠标滚动事件
            //begin
            float scrollDelta = Mouse.current.scroll.ReadValue().y;
            if (scrollDelta > 0)
                inputForward = this._mouseScrollSpeed;
            ...
            //@reviser lijuhong 注释掉inputSpeedChange
            ...
            //@reviser lijuhong 判断鼠标右键是否按下
            bool enableRotation = this._enableRotation && Input.GetMouseButton(1);
            ...
            //@reviser lijuhong 修改为上方定义的enableRotation
            if (enableRotation)
            {
            ...
            {
                //@reviser lijuhong 修改inputSpeedChange为0.0f
                this.HandleSpeedChange(0.0f);
            }

            if (this._enableMovement)
            {
                //@reviser lijuhong 修改Move调用代码
                //this.Move(movementInput);
                this.Move(inputForward, inputRight, 0);
                this.VerticalMove(inputUp);
                this.PanMove(inputPanForward, inputPanRight);
            }
        }
        ...
        private void HandleSpeedChange(float speedChangeInput)
        {
            ...
            //@reviser lijuhong 注释掉speedChangeInput
        ...
        private void Rotate(float horizontalRotation, float verticalRotation)
        {
            ...
            //@reviser lijuhong 修改为rotationSpeed属性
            float valueX = verticalRotation * this._rotationSpeed * Time.smoothDeltaTime;
            float valueY = horizontalRotation * this._rotationSpeed * Time.smoothDeltaTime;
        ...
        //@reviser lijuhong 重写移动相关代码
        //begin
        private void movement(Vector3 movementDirection)
        {
            Vector3 velocity = movementDirection * this._maxSpeed;
        ...
        /// 沿相机自身方向进行的移动
        /// </summary>
        /// <param name="forwardSpeed">向前移动的速度，为负则表示向后移动</param>
        /// <param name="rightSpeed">向右移动的速度，为负则表示向左移动</param>
        /// <param name="upSpeed">向上移动的速度，为负则表示向下移动</param>
        public void Move(float forwardSpeed, float rightSpeed, float upSpeed)
        {
            if (forwardSpeed == 0 && rightSpeed == 0 && upSpeed == 0)
                return;
            Vector3 forward = this.transform.forward * forwardSpeed;
            Vector3 right = this.transform.right * rightSpeed;
            Vector3 up = this.transform.up * upSpeed;
            //Debug.Log("Move: forwardSpeed=" + forwardSpeed + "; forward=" + forward + "; rightSpeed=" + rightSpeed + "; right=" + right + "; upSpeed=" + upSpeed + "; up=" + up);
        ...
        /// 沿地球垂直方向的移动
        /// </summary>
        /// <param name="speed">移动速度，为正表示向上移动，为负表示向下移动</param>
        public void VerticalMove(float speed)
        {
            if (speed == 0 || this._georeference == null || this._globeAnchor == null)
                return;

            double3 upUnity =
                this._georeference.GeodeticSurfaceNormalInUnity(this._globeAnchor.positionGlobeFixed);
            //Debug.Log("VerticalMove: speed=" + speed + "; up=" + upUnity);
            movement((float3)upUnity * speed);
        }
        ...
        /// <param name="forwardSpeed">向前移动的速度，为负则表示向后移动</param>
        /// <param name="rightSpeed">向右移动的速度，为负则表示向左移动</param>
        public void PanMove(float forwardSpeed, float rightSpeed)
        {
            if ((forwardSpeed == 0 && rightSpeed == 0) || this._globeAnchor == null)
                return;

            Quaternion rotationEastUpNorth = this._globeAnchor.rotationEastUpNorth;
            Vector3 eulerAngles = rotationEastUpNorth.eulerAngles;
            Quaternion rotation = Quaternion.Euler(0, eulerAngles.y, 0);
            Vector3 forward = rotation * Vector3.forward * forwardSpeed;
            Vector3 right = rotation * Vector3.right * rightSpeed;
            //Debug.Log("PanMove: forwardSpeed=" + forwardSpeed + "; forward=" + forward + "; rightSpeed=" + rightSpeed + "; right=" + right);
            movement(forward + right);
        }

        public void PanMoveForward(float speed)
        {
            this.PanMove(speed, 0);
        }

        public void PanMoveRight(float speed)
        {
            this.PanMove(0, speed);
        }
        //end
        ...
        private bool GetDynamicSpeed(out bool overrideSpeed, out float newSpeed)
        {
            ...
            //@reviser lijuhong 直接获取当前相机高度
            float height = (float)this._globeAnchor.longitudeLatitudeHeight.z, viewDistance;

            // Raycast from the camera to the Earth's center and compute the distance.
            // Ignore the result if the height is approximately 0.
            //@reviser lijuhong 不再使用RaycastTowardsEarthCenter获取height
