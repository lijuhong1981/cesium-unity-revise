...
namespace CesiumForUnity
{
    ...
    public partial class CesiumGeoreference : MonoBehaviour
    {
        ...
        //@reviser lijuhong 新增功能
        #region lijuhong additional codes

        private CesiumCameraController _cameraController;
        public CesiumCameraController cameraController => this._cameraController;
        private CesiumFlyToController _cameraFlyToController;
        public CesiumFlyToController cameraFlyToController => this._cameraFlyToController;

        private void Awake()
        {
            this._cameraController = GetComponentInChildren<CesiumCameraController>();
            if (this._cameraController == null)
                Debug.LogError("Not found CesiumCameraController component, please check the scene configuration.");
            if (this._cameraController != null)
            {
                this._cameraFlyToController = this._cameraController.GetComponent<CesiumFlyToController>();
                if (this._cameraFlyToController == null)
                    Debug.LogError("Not found CesiumFlyToController component in camera, please check the camera configuration.");
            }
        }

        /// <summary>
        /// 获取地球上任意位置的大地测量法线在Unity中的数值
        /// </summary>
        /// <param name="earthCenteredEarthFixed">The ECEF coordinates in meters.</param>
        /// <returns>The corresponding Unity coordinates.</returns>
        public double3 GeodeticSurfaceNormalInUnity(double3 earthCenteredEarthFixed)
        {
            double3 upECEF = CesiumWgs84Ellipsoid.GeodeticSurfaceNormal(earthCenteredEarthFixed);
            return this.TransformEarthCenteredEarthFixedDirectionToUnity(upECEF);
        }

        private void assignmentPickedResult(CesiumPickedResult result, RaycastHit hit, bool pickFeature)
        {
            result.distance = hit.distance;
            result.unityPosition = hit.point;
            result.transform = hit.transform;
            result.earthCenteredEarthFixed = this.TransformUnityPositionToEarthCenteredEarthFixed(new double3()
            {
                x = hit.point.x,
                y = hit.point.y,
                z = hit.point.z
            });
            result.longitudeLatitudeHeight = CesiumWgs84Ellipsoid.EarthCenteredEarthFixedToLongitudeLatitudeHeight(result.earthCenteredEarthFixed);
            if (pickFeature)
            {
                CesiumMetadata metadata = hit.transform.GetComponentInParent<CesiumMetadata>();
                if (metadata != null)
                {
                    result.features = metadata.GetFeatures(hit.transform, hit.triangleIndex);
                }
            }
        }
        ...
        /// <param name="result">输出的拾取结果对象</param>
        /// <param name="pickFeature">是否拾取CesiumFeature</param>
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromRay(Ray ray, out CesiumPickedResult result, bool pickFeature = true, float maxDistance = Mathf.Infinity)
        {
            RaycastHit hit;
            result = new();
            if (Physics.Raycast(ray, out hit, maxDistance))
            {
                assignmentPickedResult(result, hit, pickFeature);
                return true;
            }
            return false;
        }
        ...
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromRay(Ray ray, out CesiumPickedResult result, float maxDistance)
        {
            return this.PickFromRay(ray, out result, true, maxDistance);
        }
        ...
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromRay(Vector3 origin, Vector3 direction, out CesiumPickedResult result, bool pickFeature = true, float maxDistance = Mathf.Infinity)
        {
            Ray ray = new();
            ray.origin = origin;
            ray.direction = direction;
            return this.PickFromRay(ray, out result, pickFeature, maxDistance);
        }
        ...
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromRay(Vector3 origin, Vector3 direction, out CesiumPickedResult result, float maxDistance)
        {
            return this.PickFromRay(origin, direction, out result, true, maxDistance);
        }

        /// <summary>
        /// 根据射线对场景进行拾取操作
        /// </summary>
        /// <param name="originECEF">射线的ECEF坐标起点</param>
        /// <param name="directionECEF">射线的ECEF坐标方向</param>
        /// <param name="result">输出的拾取结果对象</param>
        /// <param name="pickFeature">是否拾取CesiumFeature</param>
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromRay(double3 originECEF, double3 directionECEF, out CesiumPickedResult result, bool pickFeature = true, float maxDistance = Mathf.Infinity)
        {
            Vector3 origin = (float3)this.TransformEarthCenteredEarthFixedPositionToUnity(originECEF);
            Vector3 direction = (float3)this.TransformEarthCenteredEarthFixedDirectionToUnity(directionECEF);
            return this.PickFromRay(origin, direction, out result, pickFeature, maxDistance);
        }

        /// <summary>
        /// 根据屏幕点位置对场景进行拾取操作
        /// </summary>
        /// <param name="screenPoint">屏幕点位置</param>
        /// <param name="result">输出的拾取结果对象</param>
        /// <param name="pickFeature">是否拾取CesiumFeature</param>
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromScreen(Vector3 screenPoint, out CesiumPickedResult result, bool pickFeature = true, float maxDistance = Mathf.Infinity)
        {
            if (this._cameraController == null)
            {
                result = new();
            ...
            Ray ray = this._cameraController.Camera.ScreenPointToRay(screenPoint);
            return this.PickFromRay(ray, out result, pickFeature, maxDistance);
        }
        ...
        /// <param name="maxDistance">射线检测的最大距离</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromScreen(Vector3 screenPoint, out CesiumPickedResult result, float maxDistance)
        {
            return this.PickFromScreen(screenPoint, out result, true, maxDistance);
        }
        ...
        /// <param name="pickFeature">是否拾取CesiumFeature</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromLine(Vector3 start, Vector3 end, out CesiumPickedResult result, bool pickFeature = true)
        {
            RaycastHit hit;
            result = new();
        ...
        /// 根据两个点位置对场景进行拾取操作
        /// </summary>
        /// <param name="startECEF">起点ECEF坐标位置</param>
        /// <param name="endECEF">终点ECEF坐标位置</param>
        /// <param name="result">输出的拾取结果对象</param>
        /// <param name="pickFeature">是否拾取CesiumFeature</param>
        /// <returns>拾取是否成功</returns>
        public bool PickFromLine(double3 startECEF, double3 endECEF, out CesiumPickedResult result, bool pickFeature = true)
        {
            Vector3 start = (float3)this.TransformEarthCenteredEarthFixedPositionToUnity(startECEF);
            Vector3 end = (float3)this.TransformEarthCenteredEarthFixedPositionToUnity(endECEF);
            return this.PickFromLine(start, end, out result, pickFeature);
        }
